...
SiYiCamera::SiYiCamera(QObject *parent)
    : SiYiTcpClient(QStringLiteral("192.168.144.25"), 37256, parent)
{
...
void SiYiCamera::analyzeIp(QString ip)
{
    SiYiTcpClient::analyzeIp(ip);
...
{
    yaw = qBound(-100, yaw, 100);
    pitch = qBound(-100, pitch, 100);

    constexpr uint8_t cmdId = 0x9A;
    ...
    const QByteArray msg =
        packMessage(0x01, cmdId, body);

    qInfo() << "SIYI_TURN_ENQUEUE"
            << "yaw:" << yaw
            << "pitch:" << pitch
            << "packet:" << msg.toHex(' ');
...
{
    const uint8_t cmdId = 0x9B;

    QByteArray body;
    body.append(char(0x01));

    const QByteArray msg = packMessage(0x01, cmdId, body);

    qInfo() << "SIYI_RESET_ENQUEUE"
            << "packet:" << msg.toHex(' ');
    sendMessage(msg);
    return true;
}
    ...
    while (rxBytes_.length() >= 4) {
        if ((rxBytes_.at(0) == char(0x55))
            ...
            if (rxBytes_.length() >= headerLength) {
                ...
                qInfo() << "SIYI_RX_PACKET"
                        << "ip:" << ip_
                        << "port:" << port_
                        << "cmdId:"
                        << QStringLiteral("0x%1")
                               .arg(msg.header.cmdId, 2, 16, QLatin1Char('0'))
...
{
    constexpr quint8 commandId = 0x94;
    const QByteArray message = packMessage(0x01, commandId, QByteArray());

    qInfo() << "SIYI_CAMERA_VERSION_REQUEST"
            << "ip:" << ip_
            << "port:" << port_
            << "threadRunning:" << isRunning()
            << "packet:" << message.toHex(' ');
...
{
    qInfo() << "SIYI_CAMERA_VERSION_RESPONSE"
    ...
    if (bytes4 || bytes8) {
        ...
        const quint32 rawVersion = ctx->version;
        const quint8 highByteType =
            static_cast<quint8>((rawVersion >> 24) & 0xFF);
        const quint8 lowByteType =
            static_cast<quint8>(rawVersion & 0xFF);

        qInfo() << "SIYI_CAMERA_VERSION_PARSED"
                << "rawVersion:"
                << QStringLiteral("0x%1")
                       .arg(rawVersion, 8, 16, QLatin1Char('0'))
                << "highByteType:"
                << QStringLiteral("0x%1")
                       .arg(highByteType, 2, 16, QLatin1Char('0'))
                << "lowByteType:"
                << QStringLiteral("0x%1")
                       .arg(lowByteType, 2, 16, QLatin1Char('0'));
        ...
        0x77：ZR30云台相机
        */
        auto isKnownCameraType = [](int cameraType) {
            return cameraType == CameraTypeR1
                   || cameraType == CameraTypeZR10
                   || cameraType == CameraTypeR1M
                   || cameraType == CameraTypeA8
                   || cameraType == CameraTypeA2
                   || cameraType == CameraTypeZR30
                   || cameraType == CameraTypeZT30
                   || cameraType == CameraTypeZT6;
        };

        int type = highByteType;

        if (!isKnownCameraType(type) && isKnownCameraType(lowByteType)) {
            qWarning() << "SIYI_CAMERA_TYPE_ENDIAN_FALLBACK"
                       << "highByteType:" << QStringLiteral("0x%1").arg(highByteType, 2, 16, QLatin1Char('0'))
                       << "lowByteType:" << QStringLiteral("0x%1").arg(lowByteType, 2, 16, QLatin1Char('0'));
        ...
        qInfo() << "The camera type is:"
                << QStringLiteral("0x%1").arg(type, 2, 16, QLatin1Char('0'));
        ...
        qInfo() << "SIYI_CAMERA_CAPABILITIES"
                << "type:"
                << QStringLiteral("0x%1")
                       .arg(camera_type_, 2, 16, QLatin1Char('0'))
                << "focus:" << enableFocus_
                << "zoom:" << enableZoom_
                << "photo:" << enablePhoto_
                << "video:" << enableVideo_
                << "control:" << enableControl_
                << "laser:" << m_enableLaser
                << "ai:" << m_enableAi;
        ...
        qWarning() << "SIYI_CAMERA_VERSION_INVALID_LENGTH"
                   << "packetLength:" << msg.length()
                   << "expectedBytes4:" << int(headerLength + sizeof(ACK) + 4)
                   << "expectedBytes8:" << int(headerLength + sizeof(ACKNew) + 4)
                   << "packet:" << msg.toHex(' ');
    }
}
